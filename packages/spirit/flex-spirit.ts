...
export class FlexSpirit extends ContainerSpirit {
  ...
  getchildrenMaxBottom (idx) {
    ...
    return Math.max(...this.children.filter((ele, _idx) => _idx <= idx).map(ele => ele.bottomPosition))
  }

  calculateChildSort () {
    this.children.sort((a, b) => (a.line * this.clientWidth + a.config.left) - (b.line * this.clientWidth + b.config.left))
  ...
  syncChildrenStyle () {
    if (this.children) {
      ...
      this.config.height = ((this.getchildrenMaxBottom(this.children.length) - this.config.top) || 100) + 'px'
    }
  }
}

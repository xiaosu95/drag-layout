...
export class AbsoluteSpirit extends BaseSpirit {
  ...
  checkAdsorption() {
    ...
    for (let index = 0; index < allSpiritCoordinates.x.length; index++) {
      ...
      if (
        elementX < this.config.left &&
        this.config.left - elementX < this.globalConfig.adsorptionThreshold
      ) {
        if (x > elementX) x = elementX;
      }
      if (
        elementY < this.config.top &&
        this.config.top - elementY < this.globalConfig.adsorptionThreshold
      ) {

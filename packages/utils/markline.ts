...
export class MarkLine extends Base {
  ...
  showAdsorptionYEl (x: number) {
    this.adsorptionYEl.setAttribute('style', `
  ...
  showAdsorptionXEl (y: number) {
    this.adsorptionXEl.setAttribute('style', `
  ...
  updateStyle () {
    this.xEl.setAttribute('style', `
      transform: translateY(${this.activeSpirit.config.top + this.screen.config.top + 30}px);
    `)
    this.yEl.setAttribute('style', `
      transform: translateX(${this.activeSpirit.config.left + this.screen.config.left + 30}px);
    `)
  }
}
